...
const OPENAI_DEFAULT_BASE_URL = 'https://api.sudobug.top/v1'
...
function applySudoBugOpenAIProfileDefaults(profile: ApiProfile): ApiProfile {
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const profiles = (Array.isArray(record.profiles) && record.profiles.length
    ...
    : [legacyProfile])
    .map(applySudoBugOpenAIProfileDefaults)
